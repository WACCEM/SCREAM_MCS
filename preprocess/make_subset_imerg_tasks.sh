#!/bin/bash
# This script makes slurm job array scripts to subset GPM IR+IMERG combined files within a region and period
...
# make_subset_imerg_tasks.sh input.txt
...
scratchdir="/pscratch/sd/w/wcmca1/SCREAM-cess/gpm/tb_pcp/"
exedir=$PWD

# Subset bash script
subsetscript=/pscratch/sd/w/wcmca1/SCREAM-cess/gpm/tb_pcp/ncks_pixel_file.sh
...
for iyear in $(seq $syear $eyear); do
    ...
    for imon in $(seq -f "%02g" $smonth $emonth); do
        ...
        if [[ ${infiles} ]]; then
            ...
            ls ${infiles} | awk -v dir="$ioutdir" -v lonmin="$lonmin" -v lonmax="$lonmax" -v latmin="$latmin" -v latmax="$latmax" -v subsetscript="$subsetscript" \
                '{print subsetscript, $1, dir, lonmin, lonmax, latmin, latmax}' >> ${taskname}
