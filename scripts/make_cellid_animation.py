#!/usr/bin/env python
"""
Make cell identification animation
...
# Script parameters
start_date = "2020-08-02T00"
end_date = "2020-08-03T00"
...
config_file = f"/global/homes/f/feng045/program/scream/tracking/config_cellid_3km_GridRad.yaml"

# Execution parameters
parallel_mode = 1
n_workers = 128
figsize_x = 16  # Width in inches (height is auto-calculated to maintain aspect ratio)

# Title prefix for plots (added before date/time, can be an empty string)
title_prefix = 'GridRad'

# Font sizes
fontsize = 12              # Main font size for labels and text
...
fig_basename = "cellmask_4panels_"
dpi = 240  # Figure resolution
wspace = 0.15  # Width space between subplots
hspace = 0.15  # Height space between subplots

# Output directories
figdir = f"/pscratch/sd/i/iclas2/GridRad/cell_conus/quicklooks/"
animation_dir = f"/pscratch/sd/i/iclas2/GridRad/cell_conus/animations/"
...
print("Make cell identification animation")
print(f"Plotting script: plot_cellid_convsf_echotop.py")
...
if run_plotting:
    ...
    cmd = [
        'python', 'plot_cellid_convsf_echotop.py',
        '--start', start_date,
        '--end', end_date,
        '--config', config_file,
    ]
...
if run_ffmpeg:
    ...
    for png_file in all_png_files:
        # Extract the datetime string from filename (assumes format: basename_YYYYMMDD_HHMM.png)
        try:
            basename_len = len(fig_basename)
            datetime_str = os.path.basename(png_file)[basename_len:basename_len+13]  # YYYYMMDD_HHMM
            file_dt = pd.to_datetime(datetime_str, format='%Y%m%d_%H%M')
    ...
    if len(expected_files) == 0:
        ...
        print(f"   Expected pattern: {fig_basename}YYYYMMDD_HHMM.png")
        exit(1)
